...
// Update commission submitted_at date (Admin override)
export async function updateCommissionSubmittedAt(
    id: string,
    submittedAt: string
): Promise<CommissionData | null> {
    const { data, error } = await supabaseAdmin
    ...
    if (error) {
        console.error('Error updating commission submitted_at in Supabase:', error);
        return null;
    }
...
export async function getActiveWorkloadCount(): Promise<number> {
    const { count, error } = await supabaseAdmin
        .from('commissions')
        .select('*', { count: 'exact', head: true })
        .in('status', ['pending', 'accepted', 'in_progress', 'redrawing', 'finished', 'on_delivery', 'waitlist']);
