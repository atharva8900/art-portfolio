    ...
    const content = (
        <div className={`${embedded ? 'max-w-5xl mx-auto' : 'max-w-7xl mx-auto'} ${className}`}>
            ...
            <div id="visualizer-container" className="flex flex-col lg:grid lg:grid-cols-12 gap-0 lg:gap-8 h-full items-stretch overflow-hidden bg-background">
                ...
                <div className={`lg:col-span-8 w-full shrink-0 ${isFullscreen ? 'h-[48vh]' : 'h-[42vh]'} lg:h-full flex flex-col items-center justify-center gap-4 relative bg-surface/50 lg:overflow-y-auto lg:py-8 lg:px-4`}>
                    ...
                    <div className={`relative w-full flex items-center justify-center ${orientation === 'portrait' ? 'h-full lg:aspect-[2/3] lg:h-auto lg:max-h-[500px]' : 'h-full lg:aspect-[3/2] lg:h-auto lg:max-h-[420px]'} max-h-full rounded-2xl lg:rounded-3xl overflow-hidden p-3 lg:p-8 bg-surface transition-all duration-500`}>
                        ...
                        {/* Compact Mobile Hint - Now inside the box */}
                        {image && (
                            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 lg:hidden flex items-center gap-1.5 text-[9px] uppercase tracking-[0.2em] text-foreground/40 font-bold z-30 pointer-events-none">
                ...
                <div className="lg:col-span-4 flex-1 lg:h-full lg:min-h-0 bg-surface border-t-0 lg:border lg:border-foreground/5 rounded-t-3xl lg:rounded-3xl lg:self-stretch shadow-[0_-8px_30px_rgba(0,0,0,0.4)] lg:shadow-sm flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto overscroll-contain custom-scrollbar p-4 lg:p-6 space-y-5 max-h-[calc(100svh-42vh-160px)] lg:max-h-none" onWheel={(e) => e.stopPropagation()}>
